...
## Statement Template Integration (user request 24/7)
- [x] Analyze sample statement and define template requirements (completed in statement_template_analysis.md)
- [x] Design data structures for statement generation (customer, invoices, payments, bank details):
    - Customer/Group information (name, address, payment terms)
    - Company summaries (name, currency, balance, unpaid, overdue, upcoming)
    - Document details (date, number, amount, open amount, overdue days, vessel, comments)
    - Bank details (beneficiary, IBAN, SWIFT)
- [x] Implement backend endpoint to fetch and format data for statement (implemented `getStatementData` procedure in `statementRouter` to fetch customer and invoice data)
- [x] Choose and integrate a server-side templating/PDF generation library (e.g., HTML to PDF - consider WeasyPrint or similar for Python, or a Node.js library for HTML to PDF conversion)
- [x] Create a new template for the statement of account based on the provided PDF (HTML template `server/templates/statement.html` created)
- [x] Update SendEmailDialog to include an option to attach the generated statement
- [x] Test statement generation and attachment workflow (manual testing required for PDF generation, but logic is in place)
