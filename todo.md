...
## CHACHACHA real-data validation (user request 4/8)
- [x] Add Annex 1 vessel "CHA CHA CHA" under Spring Marine Management
- [x] Seed contract CO2-2026-SPRING-001 with 17 products, 1 vessel, 3 x 5,650 EUR installments
- [x] BUG: equipment records are only generated by assignVessel, so vessels added at contract creation get none
- [x] Add explicit "Generate equipment" action per vessel and fleet-wide, idempotent (no duplicate serial rows)
- [x] Vitest coverage for equipment generation from the contract product list (10 tests)

## Sidebar rework + certificate expiry (user request 4/8)
...
- [x] Vitest coverage for route matching and certificate reminder thresholds
- [x] Certificate expiry: shared 60/15-day threshold helper (single source of truth)
- [x] Certificate expiry: capture cert number + expiry inside New Equipment dialog
- [x] Certificate expiry: New/Edit Certificate dialog on /ops/certificates
- [x] Certificate expiry: reminder tasks auto-created at 60 and 15 days (idempotent)
- [x] Certificate expiry: align dashboard KPI + row colours with 60/15 thresholds
